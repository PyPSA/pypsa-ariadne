...
def nodal_balance(n, carrier, regions, time=slice(None), aggregate=None, energy=True):
    ...
    for c in n.iterate_components(n.branch_components):
        for col in c.df.columns[c.df.columns.str.startswith("bus")]:
            ...
            df = c.df[c.df[col].map(n.buses.carrier).isin(carrier) & c.df[col].str.startswith(tuple(regions))]
...
def plot_nodal_balance(network, 
                       tech_colors,
                       savepath,
    ...
    end_date = end_date
    regions = regions
    period = network.generators_t.p.index[(network.generators_t.p.index >= start_date) & (network.generators_t.p.index <= end_date)]
    nb_el = nodal_balance(network, carrier = carriers, regions=regions, time=period, aggregate=['component', 'bus'], energy=True) 
    i_loads = network.loads[network.loads.carrier.isin(loads) & network.loads.bus.str.startswith(tuple(regions))].index
    ...
    # get colors
    c_neg, c_pos = [tech_colors[col] for col in df_neg.columns], [tech_colors[col] for col in df_pos.columns]
    ...
    # plot lmps
    lmps = network.buses_t.marginal_price[network.buses[network.buses.carrier.isin(carriers)].index].mean(axis=1)[period]
    ...
    filtered_handles_labels = [(h, l) for h, l in zip(handles, labels) if not l.startswith('out_')]
    handles, labels = zip(*filtered_handles_labels)
    
    # rescale the y-axis
    ax.set_ylim([1.05*df_neg.sum(axis=1).min(), 1.05*df_pos.sum(axis=1).max()])
    ax.legend(handles, labels, ncol=1, loc="upper center", bbox_to_anchor=(1.22, 1.01), title="Legend for left y-axis")
    ax2.legend(title="Legend for right y-axis",  loc="upper right")
    ax.set_ylabel("total electriyity balance [GW]")
    ax2.set_ylabel("lmp [€/MWh]")
    ax.set_xlabel("")
    ax.set_title(f"Electricity balance (model: {model_run}, period: {start_date} - {end_date})", fontsize=16, pad=15)
    ax.grid(True)
...
def plot_storage(network, 
    ...
    # storage carriers
    st_carriers = ["battery", "EV battery", "PHS", "hydro", "H2 Store"] # "battery", "Li ion",
    # generation carriers
    carriers = ["battery discharger",  "V2G", "PHS" ,"hydro", "H2" ] # "battery discharger", "V2G",
    ...
    for i, c in enumerate(st_carriers):
        ...
        if c in ["battery", "EV battery", "Li ion"]:
            ax1.plot(charge/max_stor_cap, label=c, color=tech_colors[c], marker=markers[i], markevery=[0], mfc='white', mec='black')
            
        else:
            ax2.plot(charge/max_stor_cap, label=c, color=tech_colors[c], marker=markers[i], markevery=[0], mfc='white', mec='black')

    ax1.set_title(f"State of charge of short-term storage technologies ({model_run})")
    ax2.set_title(f"State of charge of mid- and long-term storage technologies({model_run})")
